...
const META = () => [
  ...
  P("지은이: 이형근 · 정인호 · 송무준  |  발행처: 에이에스씨  |  판형: 신국판(152×225mm), 216쪽"),
...
const toc = [
  ...
  ["", "제19장. 온톨로지-지식그래프 온톨로지 — 말이 시스템이 되는 길"],
...
(async () => {
  ...
  await make("3_저자소개.docx", [
    ...
    P("ERP·MES·SCM 컨설턴트로 30년간 제조 현장의 전산화와 데이터 경영을 도왔다. 다품종소량·수주생산 공장에서 " +
      ...
      "컨설팅하고 있다."),
    H2("정인호"),
    P("i7/온톨로지-지식그래프 대표 컨설턴트. 경영 사각지대의 암묵지를 발굴해 업무에 쓸 수 있는 지식으로 키우는 온톨로지 기반 " +
      "AX 방법론 온톨로지-지식그래프(JEDAIX) — 5 Space × 6 Station 워크벤치와 PTGDA 알고리즘, Ontology 기반 AI-Agent Workflow — 를 " +
  ...
  await make("6_판권지.docx", [
    ...
    P("", { spacing: { after: 400 } }),
    P("판형  신국판(152×225mm)  |  쪽수  216쪽", { center: true }),
