...
const META = () => [
  ...
  P("지은이: 이형근 · 정인호 · 송무준  |  발행처: 에이에스씨  |  판형: 신국판(152×225mm), 210쪽"),
...
const 책소개 =
  ...
  "오픈소스 Odoo를 몸체로, 온톨로지-지식그래프를 판단의 층위로 — 기록하는 ERP가 판단하는 ERP로 " +
...
const 요약 =
  ...
  "사각지대를 발굴해 실행으로 잇는 온톨로지-지식그래프와 성공사례·첫 90일 로드맵이다. AI가 기업을 " +
...
const toc = [
  ...
  ["", "제16장. 확장 부스터: 지식그래프 아키텍처 구축"],
  ["", "제17장. 2부 결산: 모듈 도입 우선순위와 전사 컨설팅 로드맵"],
  ["제3부. 온톨로지 메커니즘으로 완성하는 AX — 온톨로지-지식그래프 변환기와 성공사례", ""],
  ["", "제18장. 경영 사각지대 — ERP가 보지 못하는 곳"],
  ["", "제19장. 온톨로지-지식그래프 — 말이 시스템이 되는 길"],
...
(async () => {
  ...
  await make("3_저자소개.docx", [
    H1("저자소개"), ...META(),
    H2("이형근"),
    P("ERP·MES·SCM 컨설턴트로 제조 현장의 데이터(DX)와 AX 경영을 하고 있다. LGCNS, 스마트 추진단을 거쳐 " +
      "현재 에이에스씨에서 전라권 Odoo 기반 중소기업 지능형 ERP를 컨설팅하고 있다."),
    H2("정인호"),
    P("ERP·MES·SCM 컨설턴트로 업무에 쓸 수 있는 지식으로 키우는 온톨로지 기반 AX 방법론을 설계·구축했다. " +
      "한국IBM, 영림원을 거쳐 현재 에이에스씨에서 서울·경기·충남권 Odoo 기반 중소기업 지능형 ERP를 컨설팅하고 있다."),
    H2("송무준"),
    P("ERP·MES·SCM 컨설턴트로 제조 현장의 전산화와 데이터(DX) 경영을 해왔다. LGCNS, 산업현장교수, " +
      "스마트공장 평가위원, AI코칭 위원을 거쳐 현재 에이에스씨에서 부산 및 영남권 Odoo 기반 중소기업 지능형 ERP를 " +
      "컨설팅하고 있다."),
  ]);
  ...
  await make("6_판권지.docx", [
    ...
    P("ISBN  979-11-999591-4-9", { center: true }),
    P("정가  25,000원", { center: true }),
    P("", { spacing: { after: 800 } }),
