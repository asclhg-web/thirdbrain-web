...
| 소크 테스트(3일 연속) | **완료** | E2E 재구축 53.5s(풀링·수정 후 — SQLite 64s보다 빠름) 후 야간 사이클 3일 연속 **17/17·17/17·20/20 전 단계 성공** |
| [P2-I11] 자기 환류 오염 | **수정** | 소크가 적발: 승인→발주 초안(PO/AXP/*)이 CDC로 재수집되어 lot 없는 조달→dim NaN 키→PG COPY 붕괴→하류 연쇄. 수정 4중: COPY NaN→NULL 정규화 · dim 널 키 방어(경보) · CDC/정합의 초안 제외 · 카드 테이블 자기 보장. 회귀 테스트 고정 |
| [P2-I12] 원천 테이블 부재 견고성 | **수정** | quality/maintenance 모듈 없는 Odoo에서 sync/reconcile이 죽던 것 → 우아한 건너뛰기(P-02 대응) |

**2단계 실행 종결** — 테스트 51/51(양쪽), 야간 3일 무결 소크, 이슈 12건 전부 수정·고정.
