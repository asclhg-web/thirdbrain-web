# seg01..seg34 → 단일 패스 xfade 체인 + loudnorm → 최종 mp4
import subprocess
from pathlib import Path
SP=Path(__file__).parent; AV=SP/"tsd_video"/"av"; X=0.5
segs=sorted(AV.glob("seg*.mp4")); assert len(segs)==34, len(segs)
def probe(f):
...
fc=";".join(vf+af)
final=SP/"AX_베이커리T사_통합_발표영상.mp4"
