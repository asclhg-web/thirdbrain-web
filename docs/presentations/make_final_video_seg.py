# 최종 발표영상 — 슬라이드 프레임 + 아나운서 TTS(문장별·쉼표 미세 쉼) → 슬라이드별 seg
import json, re, subprocess, wave, struct
from pathlib import Path
import sherpa_onnx

SP = Path(__file__).parent
FR = SP / "final_video" / "frames"
AV = SP / "final_video" / "av"; AV.mkdir(parents=True, exist_ok=True)
MODEL = SP / "tts-model"
...
NARR=json.loads((SP/"final_narration.json").read_text())
