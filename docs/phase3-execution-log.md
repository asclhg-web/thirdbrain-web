...
| P3-I8 | 스키마 일괄 정리에서 `LIKE 'ax_%'`의 `_` 와일드카드가 axp_prod(매핑 뷰 스키마)까지 삭제 — 정리 스크립트의 전형적 함정 | 매핑 SQL이 멱등이라 즉시 재적용으로 복구. 운영 정리 절차에는 `LIKE 'ax\_%'` 이스케이프 명기 (**해결·교훈 기록**) |
| P3-I9 | 실 Odoo 발주 라인엔 로트가 없어(로트는 입고에서 생김) fact_procurement의 NOT NULL·PK lot_id가 즉시 붕괴 — demo 데이터엔 늘 로트가 있어 숨어 있던 결함. 첫 수정(단일 'LOT-미상')도 PK 중복으로 재실패 | 자재·공급사별 유일 표식 `LOT-미상:<자재>:<공급사>`로 차원·사실 일관 적재(입고 확정 시 backfill 재처리 대상 명시) — **실 Odoo 발원 데이터로 야간 파이프라인 전 단계 정상 완주** (**해결**) |

## prod 적재기(sync_prod) — 복제 뷰 → 스테이징 결선 (2026-09-11)

2단계까지 없던 실결손을 발견·해소: demo는 sqlite 폴링(sync)이 있었지만,
**prod 경로(복제 매핑 뷰 → 스테이징)를 옮기는 적재기가 없었다.**

- `odoo_cdc.sync_prod()` 신설 — axp_prod.v_* 6종을 cdc_state 체크포인트로 증분
  폴링(멱등), 뷰 부재 시 -1 표식(모듈 부재·복제 미구성 우아한 스킵).
- 스케줄러 `AXP_CDC=prod` 스위치 결선(reconcile은 demo 전용임을 명시).
- **실측**: 실복제 데이터 1차 19행 적재 → 2차 0행(멱등). 매핑 뷰에 ::text 캐스트
  보강(스테이징 TEXT 타입 정합). PG 전용 테스트 2종 추가 — **58/58(PG)·56+2skip(sqlite)**.
- **실 Odoo 발원 전 구간 완주**: sync_prod → transform(P3-I9 수정 후) → 품질 →
  특징량 → 그래프 → 에이전트(이력 1일이라 모델 없음 경보는 정상 우아한 실패) →
  브리핑·War Room까지 `cycle: 전 단계 정상`.

## CDC 정합 소크 (진행 중 — 12주 계획 W2 "정합 무경보" 항목)

매 사이클 실 Odoo ORM으로 판매 3건(확정)·발주 1건·정비요청 1건을 만들고
9개 복제 테이블의 건수+수량 합계를 원장과 전수 대조한다(cdc_soak.py).

| 사이클 | 시각(UTC) | 생성 | 정합 | 슬롯 지연 |
|---|---|---|---|---|
| 1 | 09-11 20:55 | SO3/PO1/MR1 | **전 계열 일치** | 198 kB |
| 2 | 09-11 21:00 | SO3/PO1/MR1 | **전 계열 일치** | 176 kB |
| 3 | 09-11 21:03 | SO3/PO1/MR1 | **전 계열 일치** | 70 kB |
| 4 | 09-11 21:10 | SO3/PO1/MR1 | **전 계열 일치** | 232 kB |

**종결 판정(1차)**: 4사이클 연속 9계열 건수+수량 전수 일치·무경보, 슬롯 지연은
kB 수준에서 배수됨. 12주 계획 W2의 "정합 무경보" DoD를 이 환경 조건에서 충족.
60분 체크인 감시는 계속 유지한다(이상 발생 시 P3-I로 기록).

이후 사이클은 60분 간격 체크인으로 지속 — 종결 시 결과를 이 표에 누적한다.
